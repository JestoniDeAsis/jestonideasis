...
namespace VitalHealth.Forms.Nurse
{
    public partial class PatientsCheckUpForm : Form
    {
        ...
        /// <summary>
        /// Checks DB: does this patient already have doctor-filled patient_records?
        /// Returns true = Returning, false = New.
        /// </summary>
        /// <summary>
        /// Returns true (Returning) only if the patient has at least one doctor-completed record
        /// — i.e. at least one of the 5 doctor fields is non-empty.
        /// A patient who only has nurse-entry rows (name, vitals, reason) is still considered New.
        /// </summary>
        private bool PatientHasExistingRecords(string firstName, string lastName, string contactNumber)
        {
            var db = new DBConnection();
            using var conn = db.GetConnection();
            ...
            string sql = @"SELECT COUNT(*) 
                   ...
                   JOIN patient_queue   pq ON pd.patient_id  = pq.patient_id
                   JOIN patient_records pr ON pq.queue_id    = pr.queue_id
                   WHERE pd.first_name = @fn 
                     AND pd.last_name  = @ln
                     AND (@cn = '' OR pd.contact_number = @cn)
                     AND (
                           TRIM(pr.symptoms_report)  != ''
                        OR TRIM(pr.physical_exam)     != ''
                        OR TRIM(pr.diagnosis)         != ''
                        OR TRIM(pr.treatment_plan)    != ''
                        OR TRIM(pr.prescription)      != ''
                     )";
            try
            {
                conn.Open();
        ...
        private void rdBttn_newPatient_CheckedChanged(object sender, EventArgs e)
        {
            ...
            string cn = txtB_contactNumber.Text.Trim();

            // Can't validate without at least a name — allow selection for now
            if (string.IsNullOrWhiteSpace(fn) || string.IsNullOrWhiteSpace(ln)) return;

            if (PatientHasExistingRecords(fn, ln, cn))
            {
                MessageBox.Show(
                ...
                // Revert without re-triggering this event
                rdBttn_returningPatient.CheckedChanged -= rdBttn_returningPatient_CheckedChanged;
        ...
        private void rdBttn_returningPatient_CheckedChanged(object sender, EventArgs e)
        {
            ...
            // Name is required to search
            if (string.IsNullOrWhiteSpace(fn) || string.IsNullOrWhiteSpace(ln))
            {
                MessageBox.Show(
                    "Please enter the patient's name before selecting their patient type.",
                    "Name Required",
                    MessageBoxButtons.OK,
        ...
        private void bttn_PrintQueuingNum_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    int existingId = GetExistingPatientId(txtB_firstname.Text.Trim(), txtB_lastname.Text.Trim(), txtB_contactNumber.Text.Trim());
        ...
        private int GetExistingPatientId(string fn, string ln, string cn) { var db = new DBConnection(); using var conn = db.GetConnection(); string sql = "SELECT patient_id FROM patient_data WHERE first_name=@fn AND last_name=@ln AND contact_number=@cn LIMIT 1"; using var cmd = new MySqlCommand(sql, conn); cmd.Parameters.AddWithValue("@fn", fn); cmd.Parameters.AddWithValue("@ln", ln); cmd.Parameters.AddWithValue("@cn", cn); conn.Open(); object res = cmd.ExecuteScalar(); return res != null ? Convert.ToInt32(res) : 0; }
