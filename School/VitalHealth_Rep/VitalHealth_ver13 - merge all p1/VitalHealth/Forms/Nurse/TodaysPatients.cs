...
namespace VitalHealth.Forms.Nurse
{
    public partial class TodaysPatients : Form
    {
        ...
        private void TodaysPatients_Load(object sender, EventArgs e)
        {
            ...
            // Add "All" to the top of the list if it's not there
            ...
            LoadTodaysPatients("All");
            UpdatePatientCount("All");
        }

        // Added parameter 'specialtyFilter' so the method knows what to filter
        private void LoadTodaysPatients(string specialtyFilter = "All", string searchTerm = "")
        {
            _allPatients.Clear();
            ...
            try
            {
                ...
                string sql = @"SELECT pq.queue_id, pq.patient_id, pq.queue_number, pd.first_name, pd.last_name, 
               DATE_FORMAT(pq.queued_at, '%h:%i %p') AS ArrivalTime, 
               CONCAT('Dr. ', up.first_name, ' ', up.last_name) AS DoctorName, 
               ds.specialty_name, qs.status_name, 
        ...
        private void UpdatePatientCount(string specialty = "All")
        {
            ...
            if (specialty != "All" && !string.IsNullOrEmpty(specialty))
            {
                sql += " AND ds.specialty_name = @spec";
            }
            ...
            try
            {
                ...
                if (specialty != "All") cmd.Parameters.AddWithValue("@spec", specialty);
        ...
        private void PerformSearchAndFilter()
        {
            ...
            if (string.IsNullOrEmpty(selectedSpecialty)) selectedSpecialty = "All";

            string searchTerm = txtB_searchBar.Text;

            LoadTodaysPatients(selectedSpecialty, searchTerm);
        ...
        private void bttn_paid_Click(object sender, EventArgs e)
        {
            if (dtgView_todaysPatients.Grid.SelectedRows.Count > 0)
            {
                ...
                {
                    conn.Open();
                    string sql = "UPDATE patient_queue SET queue_status_id = 5 WHERE queue_id = @qid";

                    using var cmd = new MySqlCommand(sql, conn);
                    cmd.Parameters.AddWithValue("@qid", qId);
                    cmd.ExecuteNonQuery();

                    MessageBox.Show($"Payment confirmed for {patientName}. Status updated to 'Done'.", "Success");
                    PerformSearchAndFilter();
                }
                catch (Exception ex)
                {
