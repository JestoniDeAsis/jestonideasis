...
namespace VitalHealth
{
    public partial class MainForm : Form
    {
        ...
        private void AutoMarkMissedPatients()
        {
            ...
            try
            {
                ...
                string sql = @"
                    ...
                    SET queue_status_id = 6
                    WHERE queued_at < CURDATE()
                      AND queue_status_id IN (1, 2, 3);
                ";

                using var cmd = new MySqlCommand(sql, conn);
                int affectedRows = cmd.ExecuteNonQuery();

                System.Diagnostics.Debug.WriteLine($"Missed patients updated: {affectedRows}");
            }
            catch (Exception ex)
            {
                MessageBox.Show("Auto Missed Error: " + ex.Message);
            }
        }
    }
}
