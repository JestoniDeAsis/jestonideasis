...
namespace VitalHealth.Forms.Nurse
{
    public partial class PatientsCheckUpForm : Form
    {
        ...
        private void bttn_PrintQueuingNum_Click(object sender, EventArgs e)
        {
            ...
            // Weight (kg: realistic human range)
            if (string.IsNullOrWhiteSpace(txtB_weight.Text) || !decimal.TryParse(txtB_weight.Text, out decimal weight) || weight <= 0 || weight > 500)
            {
                MessageBox.Show("Weight must be between 1 and 500 kg.", "Validation Error");
                return;
            }

            // Heart Rate (bpm)
            if (string.IsNullOrWhiteSpace(txtB_heartRate.Text) || !int.TryParse(txtB_heartRate.Text, out int heartRate) || heartRate < 30 || heartRate > 220)
            {
                MessageBox.Show("Heart rate must be between 30 and 220 bpm.", "Validation Error");
                return;
            }

            // Temperature (Celsius)
            if (string.IsNullOrWhiteSpace(txtB_temperature.Text) || !decimal.TryParse(txtB_temperature.Text, out decimal temp) || temp < 30 || temp > 45)
            {
                MessageBox.Show("Temperature must be between 30°C and 45°C.", "Validation Error");
                return;
            }
            ...
            if (string.IsNullOrWhiteSpace(txtB_bloodPressure.Text) ||
                bpParts.Length != 2 ||
                !int.TryParse(bpParts[0], out int systolic) ||
                !int.TryParse(bpParts[1], out int diastolic) ||
                systolic < 70 || systolic > 250 ||
                diastolic < 40 || diastolic > 150)
            {
                MessageBox.Show("Blood pressure must be valid (e.g., 120/80) and within realistic range.", "Validation Error");
                return;
            }
            ...
            try
            {
                ...
                try
                {
                    ...
                    // 🚫 BLOCK if patient has unpaid "To Pay"
                    if (HasUnpaidToPayRecord(p_id, conn, trans))
                    {
                        MessageBox.Show(
                            "This patient has an unpaid record.\nPlease complete payment first before proceeding.",
                            "Unpaid Record Found",
                            MessageBoxButtons.OK,
                            MessageBoxIcon.Warning);
        ...
        private bool HasUnpaidToPayRecord(long patientId, MySqlConnection conn, MySqlTransaction trans)
        {
            string sql = @"
                SELECT COUNT(*)
                FROM patient_queue
                WHERE patient_id = @pid
                  AND queue_status_id = 4"; // 4 = To Pay
